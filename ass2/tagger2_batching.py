...
def process_line_to_lower(line):
    return [[tok.lower(), tag] for tok, tag in [pair.split() for pair in line.split("\n")]]
...
if __name__ == '__main__':
    ...
    wordVectors_.append(np.mean(wordVectors_, axis=0))
    wordVectors_.append(np.mean(wordVectors_, axis=0))
    ...
    network = OurNetwork(m, DIM=DIM, VOCAB_SIZE=VOCAB_SIZE, out_dim=out_dim, mid_dim=mid_dim, word_vectors=wordVectors)
